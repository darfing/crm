	...
	<script type="text/javascript">
		$(function(){
			...
			$("#queryActivityBtn").click(function () {
				...
				//发送请求
				$.ajax({
					url:'workbench/clue/queryActivityWithClueId.do',
					data:{
					...
					success:function (data) {
						//遍历data，显示搜索到的市场活动列表
						if(data.code=="1") {
							if(${empty data.retDara}) {
								alert("该线索暂时没有关联的市场活动");
								return;
							}
							var htmlStr = "";
			...
			$("#saveConvertClueBtn").click(function () {
				...
				var stage=$("#stage").val();
				var activityId=$("#activityName").val();
		...
		<div class="form-group" style="width: 400px;position: relative; left: 20px;">
			...
			<select id="activityName"  class="form-control">
				...
				<c:if test="${empty activityList}">
					<option>该线索暂时没有关联的市场活动</option>
				</c:if>
				<c:forEach items="${activityList}" var="a">
					<option value="${a.id}">所有者:${a.owner} 名称:${a.name} 开始时间:${a.startDate} 结束时间:${a.endDate}</option>
				</c:forEach>
			</select>
		</div>
	</form>
